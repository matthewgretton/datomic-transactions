(ns datomic-compare.transactions
  ...
  (:require [datomic-compare.util.bimap :as bimap]
            [datomic-compare.util.core :as core]))
...
(defn ids-valid? [expected-id expected-ident actual-id actual-ident]
  ;(clojure.pprint/pprint [expected-id expected-ident actual-id actual-ident])
...
(defn make-item-ids-consistent
  "Make actual entity ids consistent with expected entity ids"
  ([actual-entity expected-entity]
   (make-item-ids-consistent actual-entity expected-entity (bimap/create-empty)))
...
(defn make-ids-consistent [actual-entities expected-entities]
  ...
  (->> (map vector actual-entities expected-entities)
       (reduce (fn [[exp-act-id-bi-map output-coll] [actual expected]]
                 (let [[new-exp-act-id-bi-map output] (make-item-ids-consistent actual expected exp-act-id-bi-map)]
